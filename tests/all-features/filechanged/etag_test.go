package filechanged_test

import (
	"fmt"
	"net/http"
	"testing"

	"github.com/stretchr/testify/assert"

	"github.com/omalloc/tavern/pkg/e2e"
	xhttp "github.com/omalloc/tavern/pkg/x/http"
)

func TestETagChange(t *testing.T) {
	f := e2e.GenFile(t, 1<<20)
	f2 := e2e.GenFile(t, 1<<20)
	t.Run("test etag get old file", func(t *testing.T) {
		case1 := e2e.New("http://sendya.me.gslb.com/cases/etag/file1", e2e.RespCallbackFile(f, func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("X-Case", "old-file")
		}))
		defer case1.Close()

		rr := xhttp.NewRequestRange(1, 400000)

		resp, err := case1.Do(func(r *http.Request) {
			r.Header.Set("Range", rr.String())
		})

		hashBody := e2e.HashBody(resp)
		hashFile := e2e.HashFile(f.Path, int(rr.Start), int(rr.End))

		assert.NoError(t, err, "response should not error")

		assert.Equal(t, http.StatusPartialContent, resp.StatusCode, "response should be code 206")

		assert.Equal(t, rr.ContentRange(uint64(f.Size)), resp.Header.Get("Content-Range"), "response should be Content-Range")

		assert.Equal(t, "old-file", resp.Header.Get("X-Case"), "response should be X-Case old-file")

		assert.Equal(t, f.MD5, resp.Header.Get("ETag"), "response should be ETag")

		assert.Contains(t, resp.Header.Get("X-Cache"), "MISS", "response should be X-Cache MISS")

		assert.Equal(t, hashBody, hashFile, "response body should be equal to file MD5")
	})

	// TODO: 拼接 body 分段无法解决 etag 变更头部信息重写
	t.Run("test etag get new file", func(t *testing.T) {
		case1 := e2e.New("http://sendya.me.gslb.com/cases/etag/file1", e2e.RespCallbackFile(f2, func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("X-Case", "new-file")
		}))
		defer case1.Close()

		resp, err := case1.Do(func(r *http.Request) {
			r.Header.Set("Range", fmt.Sprintf("bytes=%d-", 600000))
		})

		assert.NoError(t, err, "response should not error")

		assert.Equal(t, http.StatusPartialContent, resp.StatusCode, "response should be code 206")

		assert.Equal(t, "old-file", resp.Header.Get("X-Case"), "response should be X-Case old-file")

		assert.Equal(t, f.MD5, resp.Header.Get("ETag"), "response should be ETag") // check is old file
	})

	// 上一个请求已经将 old-file discard; 所以这里理应当是 new-file
	t.Run("test etag get new file", func(t *testing.T) {
		case1 := e2e.New("http://sendya.me.gslb.com/cases/etag/file1", e2e.RespCallbackFile(f2, func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("X-Case", "new-file")
		}))
		defer case1.Close()

		resp, err := case1.Do(func(r *http.Request) {
		})

		hashBody := e2e.HashBody(resp)
		hashFile := e2e.HashFile(f2.Path, 0, f2.Size)

		assert.NoError(t, err, "response should not error")

		assert.Equal(t, http.StatusOK, resp.StatusCode, "response should be code 200")

		assert.Equal(t, "new-file", resp.Header.Get("X-Case"), "response should be X-Case new-file")

		assert.Equal(t, f2.MD5, resp.Header.Get("ETag"), "response should be ETag") // check is new-file

		assert.Equal(t, hashBody, hashFile, "response body should be equal to file MD5")
	})

	t.Run("PURGE", func(t *testing.T) {
		e2e.Purge(t, "http://sendya.me.gslb.com/cases/etag/file1")
	})
}
